package com.jdai.basicAuth.util;

import com.jdai.basicAuth.bean.ResourceBean;
import com.jdai.basicAuth.bean.RoleBean;
...
public class TestData {
    ...
    private List<UserBean> getAllUser(){
        ...
        ResourceBean salaryResource = new ResourceBean("2", "salaray");
    ...
    public UserBean queryUser(UserBean user){
        List<UserBean> allUsers = this.getAllUser();

        List<UserBean> userList = allUsers.stream().
                filter(userBean -> userBean.getUserName().equals(user.getUserName())
